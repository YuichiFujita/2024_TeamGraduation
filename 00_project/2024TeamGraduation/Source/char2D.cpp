...
//	コンストラクタ
//============================================================
CChar2D::CChar2D() : CObject2D(PRIORITY, LAYER::LAYER_2D),
...
//	大きさの設定処理
//============================================================
void CChar2D::SetSize(const D3DXVECTOR2& /*size*/)
{
	/*
		こっちで大きさを自由に変えられると
		文字の比率がおかしくなるのと、
...
{
	int nTexID = GetIdxTexture();	// フォントのテクスチャインデックス
	float fTexWidth = UtilFunc::Transformation::GetTexWidthFromAspect(fHeight, nTexID).x;	// テクスチャ横幅
	D3DXIMAGE_INFO status = CTexture::GetInstance()->GetTextureInfo(nTexID).imageInfo;		// テクスチャステータス
	...
	// 大きさを設定
	CObject2D::SetSize(D3DXVECTOR2(fTexWidth, fHeight));
}
