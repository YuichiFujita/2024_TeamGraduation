...
private:
	...
	// 関数リスト
	//=============================
	typedef void(CPlayerAIControlLeft::* AREA_FUNC)();
	static AREA_FUNC m_AreaFunc[];			// モード関数

	typedef void(CPlayerAIControlLeft::* MODE_FUNC)(const float, const float, const float);
	static MODE_FUNC m_ModeFunc[];			// モード関数

	typedef void(CPlayerAIControlLeft::* THROWTYPE_FUNC)(CPlayer*, const float, const float, const float);
	static THROWTYPE_FUNC m_ThrowTypeFunc[];			// 投げるタイプ関数

	typedef void(CPlayerAIControlLeft::* THROWMOVE_FUNC)(CPlayer*, const float, const float, const float);
	static THROWMOVE_FUNC m_ThrowMoveFunc[];			// 投げるまでの行動関数

	typedef void(CPlayerAIControlLeft::* THROWTIMING_FUNC)(CPlayer*, const float, const float, const float);
	static THROWTIMING_FUNC m_ThrowTimingFunc[];			// 投げタイミング関数

	typedef void(CPlayerAIControlLeft::* THROW_FUNC)();
	static THROW_FUNC m_ThrowFunc[];			// 投げタイミング関数

	typedef void(CPlayerAIControlLeft::* CATCH_FUNC)(const float, const float, const float);
	static CATCH_FUNC m_CatchFunc[];			// キャッチ関数

	typedef void(CPlayerAIControlLeft::* MOVEFORCIBLY_FUNC)();
	static MOVEFORCIBLY_FUNC m_MoveForciblyFunc[];			// 行動関数

	typedef void(CPlayerAIControlLeft::* MOVE_FUNC)();
	static MOVE_FUNC m_MoveFunc[];			// 行動関数

	typedef void(CPlayerAIControlLeft::* ACTION_FUNC)();
