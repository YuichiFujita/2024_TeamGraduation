...
float CString2D::GetStrWidth() const
{
	...
	float fHeadWidth = m_vecChar[0]->GetSize().x * 0.5f;				// 先頭文字の横幅
	fStrWidth += fHeadWidth - m_vecChar[0]->GetOffsetOrigin();			// 原点より前の空間を加算
	fStrWidth -= fHeadWidth + m_vecChar[0]->GetOffsetBlackBoxLU().x;	// ブラックボックス開始より前の空間を減算

	// 終端文字のサイズをブラックボックス右までの大きさに補正
	assert(m_vecChar[nEndCharIdx] != nullptr);
	float fTailWidth = m_vecChar[nEndCharIdx]->GetSize().x * 0.5f;				// 終端文字の横幅
	fStrWidth -= fTailWidth - m_vecChar[nEndCharIdx]->GetOffsetOrigin();		// 原点より前の空間を減算
	fStrWidth += fTailWidth + m_vecChar[nEndCharIdx]->GetOffsetBlackBoxRD().x;	// ブラックボックス終了より前の空間を加算
...
void CString2D::SetPositionRelative()
{
	...
	assert(m_vecChar[0] != nullptr);
	float fHeadOffsetOrigin	= m_vecChar[0]->GetOffsetOrigin();		// 先頭文字の原点オフセット
	float fHeadOffsetLU	= m_vecChar[0]->GetOffsetBlackBoxLU().x;	// 先頭文字のブラックボックスオフセット
	float fHeadRot		= m_vecChar[0]->GetRotation().z - HALF_PI;	// 先頭文字の向き
	float fHeadWidth	= m_vecChar[0]->GetSize().x * 0.5f;			// 先頭文字の横幅
	...
	{ // 文字数分繰り返す
		...
		assert(m_vecChar[i] != nullptr);
		float fOffset = m_vecChar[i]->GetOffsetOrigin() * 2.0f;	// 原点オフセット
		float fNext = m_vecChar[i]->GetNext() * 2.0f;			// 次文字までの距離
