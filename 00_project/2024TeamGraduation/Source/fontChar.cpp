...
HRESULT CFontChar::CreateTexture(SChar *pChar, BYTE *pBitMap)
{
	...
	MyLib::PosGrid2 sizeTexture;	// テクスチャの大きさ
	...
	MyLib::PosGrid2 sizeBlackBox = MyLib::PosGrid2(pChar->glyph.gmBlackBoxX, pChar->glyph.gmBlackBoxY);

	// ブラックボックスの中心からのオフセットを保存
	MyLib::PosGrid2 centerTexture = sizeTexture / 2;	// テクスチャの中心座標
	...
	// 生成したテクスチャのポインタを取得
	LPDIRECT3DTEXTURE9 pTexChar = pTexture->GetAdress(pChar->nTexID);
