...
{
	static float deadZone = 0.08f;
	ImGui::DragFloat("deadZone", & deadZone, 0.01f, 0.0f, 1.0f);

	bool bTipX = false, bTipY = false;
	bTipX = !UtilFunc::Calculation::IsNearlyTarget(fabsf(GetStickPositionRatioL(0).x), 0.0f, deadZone);
	bTipY = !UtilFunc::Calculation::IsNearlyTarget(fabsf(GetStickPositionRatioL(0).y), 0.0f, deadZone);
	...
	if (m_bLeftStickSelect[STICK_X] &&
		UtilFunc::Calculation::IsNearlyTarget(fabsf(GetStickPositionRatioL(0).x), 0.0f, deadZone))
	{// スティックが戻っているとき
		m_bLeftStickSelect[STICK_X] = false;
	}

	if (m_bLeftStickSelect[STICK_Y] &&
		UtilFunc::Calculation::IsNearlyTarget(fabsf(GetStickPositionRatioL(0).y), 0.0f, deadZone))
	{// スティックが戻っているとき
	...
	bTipX = !UtilFunc::Calculation::IsNearlyTarget(GetStickMoveR(0).x, 0.0f, deadZone);
	bTipY = !UtilFunc::Calculation::IsNearlyTarget(GetStickMoveR(0).y, 0.0f, deadZone);
	...
	if (m_bRightStickSelect[STICK_X] &&
		UtilFunc::Calculation::IsNearlyTarget(GetStickMoveR(0).x, 0.0f, deadZone))
	{// スティックが戻っているとき
		m_bRightStickSelect[STICK_X] = false;
	}

	if (m_bRightStickSelect[STICK_Y] &&
		UtilFunc::Calculation::IsNearlyTarget(GetStickMoveR(0).y, 0.0f, deadZone))
	{// スティックが戻っているとき
		m_bRightStickSelect[STICK_Y] = false;
	}

	// スティックのトリガーリセット
	if (UtilFunc::Calculation::IsNearlyTarget(GetStickMoveL(0).x, 0.0f, deadZone) &&
		UtilFunc::Calculation::IsNearlyTarget(GetStickMoveL(0).y, 0.0f, deadZone))
	{
		m_bLStickTip = false;
	}
	else
